...
class Noaaport:
    ...
    flh_meta = namedtuple('FrameLevelHeader', 'HDLC_address HDLC_control SBN_version SBN_control'
      ' SBN_command SBN_data_stream SBN_source SBN_destination SBN_sequence_number SBN_run'
      ' SBN_checksum')
    ...
    def open_socket(self):
        ...
        s_print(f'{datetime.utcnow()} Opening a socket to {self.ip}:{self.port} while listening to'
                f' noaaport channel {self.channel}')
    ...
    def handler(self, signum: int, frame: 'current stack frame object'):
        ...
        s_print(f'{datetime.utcnow()} handler called with signal name:{signame}, num:{signum}')
        self.stop_listening()
        os._exit(0)

    def stop_listening(self):
        s_print(f'{datetime.utcnow()} closing socket listening on.. %s:%d' % (self.ip, self.port))
        self.sock.close()
    ...
    def read_time_sync(self, time_sync_raw: bytes):
        ...
        if self.verbose: s_print(f'** Time Sync Packet: {this_date}, offset:'
                                 f' {(datetime.utcnow() - this_date)})')

    def read_frame_header(self, flh_raw: bytes, expected_sbn_seq: int) -> 'namedtuple, int':
        flh = Noaaport.flh_meta(*Noaaport.flh_struct.unpack(flh_raw))
        if expected_sbn_seq != int(flh.SBN_sequence_number):
            s_print(f'Received an SBN fragment/packet out of sequence! Expected {expected_sbn_seq}'
                    f', got {flh.SBN_sequence_number}')
        expected_sbn_seq = int(flh.SBN_sequence_number) + 1
        if self.verbose >= 3: s_print('  FLH:', [getattr(flh, i) for i in flh._fields])
    ...
    def read_sbn_header(self, sbn_raw: bytes) -> 'sbn: namedtuple':
        ...
        if self.verbose >= 2: s_print(f' flh.SBN_seq#: {self.flh.SBN_sequence_number} sbn: '
        ...
        if self.verbose >= 3: s_print('  SBN:', [getattr(sbn, i) for i in sbn._fields])
        return sbn

    def read_psh_header(self, psh_raw: bytes) -> 'psh: namedtuple':
        psh = Noaaport.psh_meta(*Noaaport.psh_struct.unpack(psh_raw))
        if self.verbose >= 3:
            s_print('    PSH:', [getattr(psh, i) for i in psh._fields], end='')
    ...
    def read_wmo_header(self, wmo_raw: bytes) -> 'wmo: namedtuple, wmo_raw: str':
        ...
        try:
            ...
            if self.verbose >= 2: s_print(f"wmo header does'nt end in '\r\r\n', {ve}. Trying '\n'")
            try:
        ...
        yearm = datetime.utcnow().strftime("%Y%m")
        ymd = datetime.strptime(yearm+ddhhmm[:6], "%Y%m%d%H%M").strftime("%Y%m%d_%H%M")
        channel = ''
        if len(ddhhmm) > 6:
            channel = ddhhmm[7:]
        wmo = Noaaport.wmo_meta(wmo_header,rstation,wmo_id,station,ddhhmm[:6],channel,ymd,wmo_size)
        if self.verbose >= 2: s_print(f'WMO Header:{wmo_header} size:{wmo_size} raw:{wmo_raw_trim}')
        if self.verbose >= 3: s_print('  WMO:', [getattr(wmo, i) for i in wmo._fields])
        return wmo
    ...
    def read_first_packet(self, data: bytes):
        ...
        wmo = self.read_wmo_header(data[wmo_start:wmo_start+40])
        ...
        if ext == 'ncf.txt':
            ...
            return
        if ext == 'none' and Noaaport.nexrad_l3_wmo_finder.search(wmo.header):
            ext = 'nexrad_l3'
    ...
    def read_data_packet(self, data: bytes):
        if self.filepath is not None:
            ...
            if (self.sbn.Transfer_type in [6, 22]):
                self.write_data()
                self.write_json()
        elif self.verbose >= 2: s_print(f' Reading data packet without the product spec / info'
                                        f' (fragment# {self.sbn.Block_number})')

    def wait_for_data(self):
        expected_sbn_sequence = 0
        while True:
            data, (host, port) = self.sock.recvfrom(65507)
            self.packet_meta = (f'{datetime.utcnow()} Received packet ({len(data)} bytes) on '
              f'{self.ip}:{self.port} (channel: {self.channel}) from {host}:{port}')
            ...
            if self.verbose: s_print(f'We shouldnt have gotten this far but..meta: '
              f'{self.packet_meta}, flh: {self.flh}; and all known cases handled.')
    ...
    def write_json(self):
        ...
        with open(self.filepath + '.json', 'w', encoding="utf-8") as json_file:
...
def human_size(fsize: str, units=[' bytes','KB','MB','GB','TB', 'PB', 'EB']):
    return f"{float(fsize):.1f}{units[0]}" if float(fsize) < 1024 else human_size(fsize / 1024, units[1:])
...
def setup_arg_parser(description: str) -> ArgumentParser:
    ...
    parser.add_argument('channel', help='Specify the channel you want to listen to',
      choices=[str(x) for x in Noaaport.channels])
...
def main():
    ...
    Noaaport(args.channel, args.dest, args.verbose)
